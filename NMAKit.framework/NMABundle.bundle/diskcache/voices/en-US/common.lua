...
--    command_id_2 = ""            -- ID for the second command
    ...
    route_option_string = ""     -- route option string
end
...
function process_car_1st_cmd( )
    ...
    elseif ( trigger == "ROUTE_CALCULATION_BEGIN" ) then
        process_route_calculation( true )

    elseif ( trigger == "ROUTE_CALCULATION_END" ) then
        process_route_calculation( false )

    elseif ( trigger == "ROUTE_ATTRIBUTES_VIOLATION" ) then
        process_route_attributes( trigger )
    ...
    elseif trigger == "TMC_NOTIFICATION" then
        process_tmc_notification( )

    elseif trigger == "OFFROAD_DESTINATION_REACHED" then
        process_offroad_target( true )

    elseif trigger == "OFFROAD_STOPOVER_REACHED" then
        process_offroad_target( false )

    elseif trigger == "ROUTE_RECALCULATION_AUTOMATIC" then
        process_route_recalculation( true )

    elseif trigger == "ROUTE_RECALCULATION_MANUAL" then
        process_route_recalculation( false )
...
function process_car_final_command( maneuver, useSignpost )
    ...
    if maneuver.id ~= "ROUNDABOUT" and maneuver.id ~= "DRIVE_TO_NEAREST_ROAD" then
...
function add_street_name_or_route( maneuver, useSignpost )
    ...
    if useSignpost and use_streetnames then
        ...
        --elseif road_class == "STREET" and ( ( show_nguidance == false ) or ( maneuver.ng_type == "TRAFFIC LIGHT" ) ) then
...
function build_streetname( street_name, r_sentence, phoneme_string_builder )
    ...
    if (phoneme == "" or phoneme == nil ) then
    ...
    if phoneme_string_builder ~= nil then
        return phoneme_string_builder( name, phoneme, lang, article )
    else
        return article .. name
    end
end
...
function build_signpost( signpost_name, r_sentence, phoneme_string_builder )
    ...
    if phoneme_string_builder ~= nil then
        return phoneme_string_builder( name, phoneme, lang, "" )
    else
        return name
    end
end
...
function filter_natural_guidance( maneuver )
    ...
    then
        ...
        elseif ( maneuver.ng_type == "STOP SIGN" and voice_settings["nguidance_stop_sign"] ) then
            show_nguidance = true
        elseif ( maneuver.ng_type == "JUNCTION" and voice_settings["nguidance_junction"] and maneuver.ng_calculated_importance ~= nil and maneuver.ng_calculated_importance >= 5 ) then
...
function build_natural_guidance( maneuver, phoneme_string_builder)
    ...
    if (maneuver.ng_type == "STOP_SIGN") then
...
function build_natural_guidance_junction( maneuver, phoneme_string_builder)
    ...
    local content = phoneme_string_builder( maneuver.ng_name, phoneme, maneuver.ng_language, '' )
    ...
    if maneuver.ng_preposition:sub(4, 4) == "B" then
        return phoneme_string_builder( maneuver.ng_name, phoneme, maneuver.ng_language, preposition )
    else
        return phoneme_string_builder( preposition, phoneme, maneuver.ng_language, maneuver.ng_name )
    end
...
phoneme_to_marc = {
    ["BRA"] = "076",     -- Brazilian Portuguese
    ["ENG"] = "010",     -- US English
    ["FRE"] = "FRE",     -- French
    ["GER"] = "GER",     -- German
    ["ITA"] = "ITA",     -- Italian
    ["NAS"] = "083",     -- North American Spanish
    ["POR"] = "POR",     -- Portuguese
    ["SPA"] = "SPA",     -- Spanish
    ["UKE"] = "ENG",     -- UK English
}
...
function process_route_calculation( is_calculation_begin )
    set_result_id( 1, "x" )
    set_result_id( 2, "r" )
    if ( is_calculation_begin == true ) then
        set_result_id( 3, "b" )
    else
        set_result_id( 3, "e" )
    end
...
-- parse the route_options string and map the corresponding options into route_option_string
...
function process_offroad_target( is_target_final )
    set_result_id( 1, "x" )
    set_result_id( 2, "o" )
    if ( is_target_final == true ) then
        set_result_id( 3, "e" )
    else
        set_result_id( 3, "s" )
    end
